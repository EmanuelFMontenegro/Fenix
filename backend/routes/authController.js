...
async function registerAdmin(user, pass, correo) {
  try {
    ...
    if (existingUser.length > 0) {
      ...
      const rol_id = 1;
      const insertQuery =
        "INSERT INTO usuarios (user, pass, correo, rol_id) VALUES (?, ?, ?, ?)";
...
async function findUserByUser(user) {
  try {
    const [rows] = await pool.query("SELECT * FROM usuarios WHERE user = ?", [
      user,
    ]);
...
async function handleForgotPasswordRequest(correo, pass) {
  try {
    ...
    if (!pass) {
      return {
        success: false,
        message: "La nueva contraseña no fue proporcionada",
      };
    }
    ...
    console.error(
      "Error al manejar la solicitud de recuperación de contraseña:",
      error
    );
    return {
      success: false,
      message: "Error interno del servidor - Detalles: " + error.message,
    };
  }
}
